...
var requirementConverters = {
  ...
  integer: function(string) {
    if (isNaN(string))
      throw 'RequirementHelper is not an integer: "' + string + '"';
    return parseInt(string, 10);
  },
  number: function(string) {
    if (isNaN(string))
      throw 'RequirementHelper is not a number: "' + string + '"';
...
var convertArrayRequirement = function(string, length) {
  ...
  if (!m)
    throw 'RequirementHelper is not an array: "' + string + '"';
  var values = m[1].split(',').map(ParsleyUtils.trimString);
  if (values.length !== length)
    throw 'RequirementHelper has ' + values.length + ' values when ' + length + ' are needed';
  return values;
};
